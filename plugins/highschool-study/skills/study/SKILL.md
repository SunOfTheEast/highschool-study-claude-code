...
5. If the student explicitly asks to prepare directly, route to `highschool-study:prepare-next-lesson` even when a prepared Lesson exists. Treat the direct request as approval to create the next indexed Lesson; do not ask whether to begin or adjust the existing prepared Lesson.
6. If a prepared next Lesson exists, ask whether to begin and route to `highschool-study:run-lesson` only on agreement.
7. If the current Plan has capability evidence and the student explicitly chooses Plan completion, route the final reflection through `highschool-study:consolidate-plan-memory`.
8. Otherwise choose the next eligible Plan from recorded dependencies and student-approved order, then route to `highschool-study:prepare-next-lesson`.
