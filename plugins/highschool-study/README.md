...
首次制定 Roadmap、选择下一学习周期和准备每节 Lesson 时，`study-coach` 会先
进行一段短问诊：每轮只问一个问题，通常连续问几个会改变教学决定的问题。下一问
沿学生刚才最宽泛的说法继续，先澄清题型、情境、卡点阶段、近期实例或实际尝试，
再判断原因，而不是让学生确认 Coach 预设的诊断。

已有摘要和 Trace 用来避免重复问已知事实，不能代替学生当下的理解与选择。Coach
会先总结并允许纠正；备课时先给出无剧透课堂意图，学生确认或调整后才确定课型、
任务作用和真实题卡。形成的判断必须有来源、改变实际 Plan 或 Lesson，并能被后续
表现支持或推翻。这只是 Skill 驱动的自然对话，不是问卷、分数或自动诊断工具。
