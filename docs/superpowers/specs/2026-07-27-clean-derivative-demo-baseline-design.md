...
只复制现有测试实际使用的内容：四张题卡，以及验证 Trace 方法节点和另解绑定所需的方法词表；不复制全部 519 张题卡或完整知识图谱。所有原本直接读取公开 Demo 的 Pi 前端测试改为读取该夹具。E2E 仍然在临时目录中复制夹具，避免测试写回仓库。
