...
架构复演中出现的四个问题已经收敛，不再作为开放问题保留：

1. **Light Lesson 的自然收口顺序**与正式 Lesson 一致：短回顾 → 先听学生 → 有边界的判断
   → 静默固化 → 自然总结 → 接受纠正 → 学生关闭。时长和是否属于 Plan 不改变这条顺序。
2. **课末断线与重复调用**采用持久 receipt、Pi tool result 恢复、关闭接口幂等和前端重连
   补读；不增加大事务或课末状态机。精确设计见
   `2026-08-08-teaching-session-close-recovery-design.md`。
3. **题卡作答、答案查看和随后问老师**是按发生顺序追加的独立事件。后来的 Light Lesson
   只能追加引用先前活动，不能把“未来会问老师”回写进旧作答事件，因此无需新建一套状态。
4. **成熟个人资产不能单独证明掌握。** Note 或题卡内容属于资产；它怎样在真实教学中形成，
   只有进入来源 Session Log、作答记录或 Trace 后才能参与教师判断。

### 6.2 仍待澄清

以下问题仍需通过真实学生流程和失败复演单独讨论，不把任何候选修复冒充为既定北极星决策：

1. **正式 Lesson 在 Uses、学生主动带入资产和个人资产索引之间的读取边界。**
2. **可修改教师题卡的历史 revision 怎样供旧作答稳定追溯。**
3. **学生同时打开多个 Light Lesson 时，学习集级 canonical 写入怎样串行化。**
4. **原始资料的局部蒸馏由当前 Tutor、Plan Coach 还是有界工作 Agent 承担，以及怎样批准
   生成可复用教学资产。**
5. **个人内容泛化为公开教学资产时，由哪类 Session 提案、核验和执行发布。**
