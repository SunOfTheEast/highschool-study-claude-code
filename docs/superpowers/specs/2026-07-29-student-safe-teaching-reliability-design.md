...
结论同时说明它当前适用的任务范围、帮助条件和样本边界，避免把“一节课中的一次
独立完成”重新概括成“整个周期零提示”或“已经稳定掌握”。

学生默认先看到自然语言结论、适用范围和下一步，需要时再展开来源，并可把异议直接
带回原 Plan Coach 对话。
...
5. 首次以 `complete` 完成 Plan 前，必须用现有 Evidence Scout 核对拟写结论的反证和
   适用范围；
6. Plan 审计和画像落盘后，只汇报重新读取到的持久化结果。
...
1. Coach 对本阶段学习结果的自然语言结论及其当前适用范围；
2. 建议的下一步。

适用范围使用“这项判断目前适用于……”一类自然表达，说明：

- 它来自哪些任务或题型；
- 学生是在独立、提示后还是其他支持条件下完成；
- 当前有多少相互独立的表现支持它；
- 哪些迁移、间隔保持或陌生情境尚未验证。

它不显示统计评分，也不要求机械凑齐四项；只保留会阻止读者过度理解当前结论的真实
边界。
...
`boundary` 是学生可见的结论组成部分，不是免责声明。它必须与关键来源和辅助来源中
已经记录的任务范围、帮助条件及样本数量一致；不得把所有不确定性统一写成空泛的
“仍需观察”。
...
在第一次以 `complete` 调用 `plan_update` 前，Plan Coach 复用现有只读 Evidence
Scout，对拟写的 `conclusion`、`boundary` 和来源分层做一次反证核对。

这次核对只回答：

- 是否遗漏了与结论冲突的 active Trace；
- 某条关键表现是否实际存在提示、课前曝光或其他支持条件；
- 结论是否把单题、单问或单一题型扩大成整个 Plan；
- 是否存在学生已经明确纠正、但拟写结论仍沿用的旧表述；
- Coach 准备声称已经写入的文件或注意事项是否能够重新读取。

Evidence Scout 只返回紧凑的问题说明、精确来源和缺失信息，不给出
active / complete / replan 判决，也不生成最终文案。Plan Coach 负责修订
`learningReview` 并作最后决定。

每次 Plan 完成默认核对一次。只有 Coach 修改了关键来源集合，才允许再核对一次；
不得通过反复启动 Scout 代替自己的教学判断。核对失败或超时时，Coach 必须如实缩小
`boundary`，并把未核实部分放入 `openQuestions`，不能声称“未发现反证”。

核对过程保留在现有 Dynamic Workflow 的原始结果中，学生界面只显示核对后的学习
回顾，不增加新的 Evidence 对象、裁判 Agent 或持久化层。
...
如果系统发生证据夸大、内部术语泄露或工具事实误报，模拟学生不以“审计员”口吻替
系统指出问题；只允许作出真实学生可能产生的自然反应。运行结束后再由独立验收者对照
学生可见对话、原始 Session 和持久化事实记录这些问题。
...
5. **学习回顾**：反证核对是否发生，结论、适用范围、关键来源、参考来源和未决问题
   是否相称；
...
| 完成前反证核对 | 复用现有只读 Evidence Scout 与 Dynamic Workflow |
...
- 阶段结论会说明自己在哪些任务和帮助条件下成立，不把一次好表现包装成稳定能力；
