...
不再固定构造第三条“方向影响不明确”路径。此前测试话术“只告诉我接下来应当从哪个方向检查”没有明确“方向”是检查对象、证明目标、方法类别还是下一步动作；更重要的是，该题学生在请求前已经明确写出“还差 `a=e^{-1}` 的充分性”，Tutor 后续只重复了这个既有目标。学生随后自行给出全部决定性步骤，应直接归为第一条 `support:none`，不能为了命中学生确认分支而强行追问。

学生确认机制仍保留，只在真实课堂中出现“Tutor 新给出方向，但数学内容来源不足以判断该方向是否促成关键步骤”时使用；本设计不再用含义不明的合成话术作为固定完成门。
...
- 两条有效的真实模型路径分别得到 `none` 与 `tutor`。
- 每条最终 Trace 都正确 supersede 当前 attempt 的 active Trace。
- 学生确认机制继续受文本契约覆盖，但不再要求用含义不明的第三条合成路径触发。
- 自动回归保持通过。
