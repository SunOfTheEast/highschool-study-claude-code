...
**日期：** 2026-08-08；2026-08-09 完成减法复核

**产品基线：** M1b 闭环 `3abab14`；正式 Lesson 增量记忆 `2c50d21`
...
```text
原始资料（Material）──Search / Read──┐
空白自由讨论 ─────────────────────────┼── 真实学习或备课 ──确认── Note / 题卡
既有学习资产 ─────────再次使用────────┘                              │
Note / 题卡 ────────────────────────────────────────────────────────┼── 平坦语义标签与可重建关系
Material revision + locator ────────────────────────────────────────┤   形成来源
自由学习 ───────────────┐                                          ├── 教师对象记忆
...
1. 新 Note 与题卡使用动态、平坦、带主次的语义标签；旧题卡通过兼容投影进入同一检索面。
2. 标签、来源和对象记忆建立可重建关系，使内容与学生认知可以互相召回而不互相冒充。
3. 书籍、PDF、图片、讲义等原始资料可以入库；Agent 通过原生 Search / Read 定位原文，并在
   真实使用中渐进沉淀为 Note 或题卡。
...
| --- | --- | --- |
| 原始资料在某个 revision 的真实内容 | Material revision | 直接 Search / Read；必要时机械提取文本 |
...
| Note 或题卡涉及哪些语义 | 独立 tag sidecar | 生成词表、邻居和召回索引 |
...
新 Note 与题卡至少有一个 `core`，`related` 可以为空。Material 不要求 tag sidecar；需要
相关原文时直接 Search / Read。模型只选择学习资产的语义词语；Runtime 绑定资产身份、时间
和 metadata revision。
...
标签集合的去重、可重建视图，不要求模型同时写一份中央词表。

遇到新词时可以直接成为 canonical 标签。M1c 不建立 alias、redirect 或本体整理工具，也不
要求 Runtime 判断两个词是否同义。检索时，Scout 可以像搜索普通文件一样使用多个短词或
`|` 展开近义表达；词语本身的语义由模型在当前任务中理解，不固化成第二套标签事实。

如果真实使用后来证明同义标签分裂已经明显损害检索或图的可读性，再依据实际数据设计归并
机制；M1c 不为尚未出现的问题预建治理系统。
...
- 旧 `graph/aliases.yaml` 继续只服务旧题库已有的查询展开，不复制成新语义事实；
...
`locator` 必须能够解析到对应 Material revision 中真实存在的位置，例如 `page-0042`、章节
标题或文本行区间；模型只使用当前 Session 已绑定的短别名，Runtime 解析并校验稳定 ID、
revision 和 locator。原文件可以直接读取时，不要求为了这项校验另建 locator index。
...
└── projections/<revision>/...   # 仅在原文件不可直接搜索时保存可重建文本与定位映射
```
...
原文件可以直接 Search / Read 时，不生成额外检索层。只有 PDF、图片等格式无法直接搜索时，
才机械提取文本或 OCR，并保留从文本位置回到页码、章节或原文件位置的 locator 映射。该投影
不生成语义标签、关键词或几百个新学习资产。投影失败时原资料仍然成功入库，并显示为“暂时
无法搜索正文”，可以机械重试。
...
直接 Search / Read 原文并返回相关 locator。讨论或备课真正形成可复用内容后，才沿现有保存
门创建 Note 或题卡，并写入准确来源 revision。
...
学生明确要求“整理整本书”时可以启动独立 Worker，按需 Search / Read 原文并生成候选草稿，
但它不是默认导入动作。候选草稿不能直接冒充 canonical 学习资产，也不更新记忆；批量资产
发布需要单独的可见复核，不属于 M1c 最小闭环的通过条件。
...
- Scout 只读取安全的标签、公开题面，并按需 Search / Read Material 原文，不读取学生笔记、
  作答或记忆；
...
M1c 不因此给所有课程角色开放通用资产写权限。自由学习保留 M1b 的 `save_note` /
`save_problem_card`；正式备课只获得上述自编题的窄确认通路；Roadmap、Meta 和普通 Plan
讨论不获得通用资产写权限；正式 Lesson 的现场生成边界见下一节。

### 8.4 正式 Lesson 可以沉淀现场形成的资产

正式 Lesson 上课时，如果学生现场形成了一段明确的解释或回忆材料，或者师生临时讨论了
备课资产之外的一道题，Tutor 可以沿用自由学习的公开保存门：
...
资产保存不会自动更新对象记忆；对象记忆也不会自动生成资产。课堂中的学生表现仍由 Lesson
Block Classroom Log 持有，资产只保存以后可继续使用的内容。没有明确确认时，Lesson 照常
进行，不因追求沉淀而打断教学。
...
Material 不进入这份标签索引；需要原始资料时，Scout 直接 Search / Read 允许访问的资料。
...
- Scout 可以用多个短词或 `|` 搜索同义、近义表达，但不把临时搜索词写成持久 alias；
...
- Roadmap、Plan、Lesson 所属 Pi Session 的首次活动、后续活动与节点当前状态；
...
不会为了修正足迹而回写。Pi Session 的 Header、每条 Entry 以及 SessionInfo 已提供真实
时间；M1c 不为显示足迹另给 Plan / Lesson 增加生命周期时间字段。节点正式关闭或完成的
精确按钮时刻没有独立事件时，只展示会话活动时间与当前状态，不伪造状态迁移时间。
...
- 正式 Lesson Tutor 获得与自由学习同门限的 Note / 题卡保存能力；
- 记忆读取仍使用原生 `Read` / `Grep`，不新增通用 recall 工具。

## 十三、原子性、修正与失败边界

1. 新 Note / 题卡内容、准确来源和初始 tag sidecar 同一次成功或全部不发生；Material 只完成
   原始入库及必要的机械可读化，不补语义 sidecar；召回索引随后可重建。
2. 标签单独更新只修改 sidecar 并增加 metadata revision，不改资产 revision。
3. stale 资产 revision 与 stale metadata revision 分别拒绝，不静默覆盖。
4. Material 原始内容保存成功但必要的文本提取或 OCR 失败时保留 Material，并明确显示正文
   暂不可搜索；重试不创建第二个 revision。
5. 新写入来源无法解析、revision 不存在、locator 不存在或形成环时拒绝资产写入，不自动换成
   最新来源。
6. 任何投影重建失败都不能损坏资产、来源、记忆、Session 或课程文档。
7. Meta 创建 Roadmap 失败时保留已确认的对话，但不留下半个 `ROADMAP.md` 或自动重试物化。
8. 自编题保存失败不回滚已经准备好的 Lesson；修复后只重试同一题卡写入与精确挂载。
9. 内容相似不会触发自动资产合并；标签相近不会触发对象记忆合并。

## 十四、三个真实情境

### 14.1 导入一本化学书，慢慢长成学习集

学生导入《化学反应原理》PDF。系统保存 Material revision；若原文件不可直接搜索，只机械
生成可搜索文本与页码映射。没有生成标签、课程、Note、题卡或“已学习”记录。
...
Note 有标签但 `sources` 为空；创建 Session 仍由 provenance 单独记录。以后导入相关教材时，
Agent 可以通过原文 Search 与 Note 标签把两者召回到同一次讨论，但既有 Note 不能事后伪造
为“来源于那本书”。真实解释同时可以支持对象记忆，漂亮 Note 本身仍不证明理解。
...
3. Scout 使用 brief 中的短词与必要的 `|` 展开，找到两道合格题后立即停止，不沿邻居寻找
   更优题；
...
| 可能的冲突 | M1c 边界 |
| --- | --- |
| 导入整本书看起来像“学了很多” | 导入只产生 Material 与必要的机械可读文本，不产生认知判断 |
...
| 一本书被预先拆成大量带标签片段 | Agent 直接 Search / Read 原文；只在形成 Note / 题卡后写标签 |
...
1. 新 Note、题卡创建时原子生成内容、pinned sources 与初始 tag sidecar；Material 不需要
   标签即可完成入库，并能直接或通过机械文本投影 Search / Read。
...
6. tag、来源和对象关系可以重建索引；删除全部投影不损坏 canonical 内容。
7. 导入资料不创建 Note、题卡、记忆、Roadmap 或掌握字段；必要的机械文本提取失败可恢复。
...
11. 足迹可从节点、Pi Session 时间、activity、资产和对象历史完全重建，没有 footprint log
    或新增节点生命周期时间字段。
12. Scout 可以用短词和 `|` 展开查询但不持久化同义词；未授权时不沿邻居，授权放宽时仍在
    达到 brief 数量后停止。
13. 自编题被拒绝保存时 Lesson 仍可上课；确认保存后只产生一张卡和一个精确 Uses 挂载。
14. 正式 Lesson 现场形成的 Note / 题卡必须经过学生确认；保存资产与更新对象记忆互不触发。
...
- Material 导入、正文可搜索状态、打开与选择 locator；
- 资产标签和准确来源的读取；
...
2. Material revision、原生 Search / Read、必要的机械文本投影与准确 locator；
...
- M1c 不建立新标签 alias / redirect 系统；Scout 使用短词和 `|` 展开查询，语义邻居只有 brief
  授权时才能放宽，并继续服从数量停止门。
- 正式 Lesson 可以在学生确认后保存现场形成的 Note / 题卡，且不与对象记忆自动绑定。
- 学习足迹使用 node-owned Pi Session 的真实时间与节点当前状态，不另建生命周期日志。
