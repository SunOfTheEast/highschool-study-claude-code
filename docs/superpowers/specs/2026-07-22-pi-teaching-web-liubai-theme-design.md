...
不加入页面级渐变、仿纸颗粒和装饰性远山。主题应保持轻、平、安静。

> 修订（2026-08-04，预览草稿 A/B 拍板）：允许限量使用朱砂印章，新增 `--seal: #9e4f3d`。朱砂只出现在品牌章与「继续学习」印章两处，永不出现在状态、错误或教学语义附近（与 `--danger` 砖红 `#a8674f` 不同色、不并排）。其余「不加入」条款不变。
...
> 修订（2026-08-04，字体对比页拍板）：阅读文本统一为霞鹜文楷 —— 标题、长文、题面、对话正文同体；等宽字体仍用于 Session ID、题卡别名、时间戳等零件。落地时加载 LXGW WenKai Screen webfont（分包按需加载），回退链为系统楷体与系统黑体。上文「双层字体策略」与「楷体不覆盖所有正文」由此条取代。
